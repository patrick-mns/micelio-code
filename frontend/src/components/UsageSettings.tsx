...
export default function UsageSettings({ onSubView }: SettingsPanelProps) {
  ...
  const [selected, setSelected] = useState<UsageLogEntry | null>(null); // ledger entry drilled into
  const [page, setPage] = useState(0); // ledger pagination (0-indexed)
  const [confirmClear, setConfirmClear] = useState(false);
  const { setActiveTab, setCurrentSession, setMessages, setShowSettings } = useStore();
  const PAGE_SIZE = 5;
  ...
  // Drilling into a row swaps this panel for the detail view, so hand the modal
  // header a back button. Cleanup also covers unmount (category switch / close).
  useEffect(() => {
    if (!onSubView) return;
    onSubView(selected ? { title: 'Turn detail', onBack: () => setSelected(null) } : null);
    return () => onSubView(null);
  }, [selected, onSubView]);
  ...
  return (
    <div>
      <div style={{ ...styles.headRow, margin: '0 0 20px' }}>
